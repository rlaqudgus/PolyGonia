...
*/
		...
		void OnSceneGUI () {
			if(light){
				...
				for (int i = 0; i <= 2 ; i++)
				{
					...
					#if UNITY_5_6_OR_NEWER
						var fmh_657_7_638506266045348839 = Quaternion.identity; newHandlePoint = (lTransform.position - Handles.FreeMoveHandle(
				...
				var fmh_750_79_638506266045368682 = Quaternion.identity; Vector3 newFreeHandlePoint = (Handles.FreeMoveHandle(lTransform.position,
		...
		*/
		...
		void PerformAndDrawAngleHandles(DynamicLight2D.DynamicLight light)
		{
			...
			#if UNITY_5_6_OR_NEWER
			var fmh_1104_53_638506266045373466 = Quaternion.identity; Vector3 cwHandle = Handles.FreeMoveHandle(cwPos, HandleUtility.GetHandleSize(light.transform.position) * .04f, Vector3.zero, Handles.CircleHandleCap);
            #else
			...
			#if UNITY_5_6_OR_NEWER
			var fmh_1121_45_638506266045375175 = Quaternion.identity; cwHandle = Handles.FreeMoveHandle(cwPos, HandleUtility.GetHandleSize(light.transform.position) * .04f, Vector3.zero, Handles.CircleHandleCap);
			#else
